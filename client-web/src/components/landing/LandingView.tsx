/**
 * LandingView — SEO-friendly landing page content (server component).
 *
 * Used by both the root page (/) and /welcome to give crawlers and first-time
 * visitors fully-rendered HTML instead of a SPA shell. CTAs link to /welcome
 * where the actual GitHub sign-in flow lives.
 */
import Link from "next/link";

interface LandingViewProps {
  /**
   * What to render in the primary CTA position. Pass a client-side sign-in
   * button here on /welcome; the root page can pass a simple <Link>.
   */
  primaryCta?: React.ReactNode;
  /** Optional secondary CTA (defaults to a "View on GitHub" link). */
  secondaryCta?: React.ReactNode;
  /** Whether to render the JSON-LD structured data block. */
        ...
        <div className="flex min-h-[220px] md:min-h-[280px]">
          <div className="hidden w-44 shrink-0 border-r border-[var(--neutral-200)] bg-[var(--neutral-50)] p-3 text-xs text-[var(--neutral-500)] sm:block">
            ...
            <div className="space-y-1">
              <div className="flex items-center gap-1.5 rounded bg-[var(--neutral-100)] px-1.5 py-1 font-medium text-[var(--foreground)]">
          ...
          <div className="flex-1 p-4 md:p-6">
            <div className="space-y-3 font-mono text-sm leading-relaxed">
              <div className="text-xl font-bold text-[var(--foreground)]"># My Knowledge Base</div>
              <div className="text-[var(--neutral-500)]">
                Welcome to my personal wiki powered by{" "}
                <span className="rounded bg-[var(--neutral-100)] px-1 py-0.5 text-[var(--foreground)]">Motion</span>.
              </div>
              <div className="text-base font-semibold text-[var(--foreground)]">## Quick Links</div>
              <div className="text-[var(--neutral-500)]">
...
const softwareApplicationSchema = {
  ...
  featureList: [
    "Agent-Friendly via built-in MCP Server",
    "Pure-frontend, privacy-first architecture",
    "Notion-style block editor with Markdown output",
    "GitHub-backed storage with offline caching",
    "Bidirectional wiki-links with force-directed graph view",
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="relative mx-auto max-w-3xl px-6 pb-8 pt-16 text-center md:pt-24">
          ...
          <p className="mx-auto mb-10 max-w-xl text-base leading-relaxed text-[var(--neutral-500)] md:text-lg">
            A pure-frontend knowledge editor where AI agents and humans collaborate. Your data lives
            in GitHub — private, versioned, and always under your control.
          </p>
        ...
        <div className="grid gap-8 sm:grid-cols-3 sm:gap-6">
          {steps.map((step, i) => (
            <div key={step.title} className="relative text-center">
              {i < steps.length - 1 && (
                <div className="pointer-events-none absolute right-0 top-10 hidden h-px w-6 translate-x-full bg-[var(--neutral-200)] sm:block" />
              )}
        ...
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {features.map((feature) => (
            ...
            >
              ...
              <p className="text-sm leading-relaxed text-[var(--neutral-500)]">{feature.description}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="border-y border-[var(--neutral-200)] bg-[var(--neutral-50)]">
        <div className="mx-auto max-w-5xl px-6 py-16">
          <h2 className="mb-3 text-center text-2xl font-bold tracking-tight md:text-3xl">
            Looking for an alternative to your current tool?
          </h2>
          <p className="mx-auto mb-10 max-w-lg text-center text-[var(--neutral-500)]">
            See how Motion compares to popular knowledge base apps.
          </p>
          <div className="mx-auto grid max-w-3xl gap-3 sm:grid-cols-2 lg:grid-cols-3">
            <Link
              href="/compare/notion-alternative"
              className="rounded-lg border border-[var(--neutral-200)] bg-[var(--background)] px-4 py-3 text-sm font-medium transition-colors hover:bg-[var(--neutral-100)]"
            >
              Motion vs Notion →
            </Link>
            <Link
              href="/compare/obsidian-alternative"
              className="rounded-lg border border-[var(--neutral-200)] bg-[var(--background)] px-4 py-3 text-sm font-medium transition-colors hover:bg-[var(--neutral-100)]"
            >
              Motion vs Obsidian →
            </Link>
            <Link
              href="/compare/logseq-alternative"
              className="rounded-lg border border-[var(--neutral-200)] bg-[var(--background)] px-4 py-3 text-sm font-medium transition-colors hover:bg-[var(--neutral-100)]"
            >
              Motion vs Logseq →
            </Link>
            <Link
              href="/compare/outline-alternative"
              className="rounded-lg border border-[var(--neutral-200)] bg-[var(--background)] px-4 py-3 text-sm font-medium transition-colors hover:bg-[var(--neutral-100)]"
            >
              Motion vs Outline →
            </Link>
            <Link
              href="/compare/anytype-alternative"
              className="rounded-lg border border-[var(--neutral-200)] bg-[var(--background)] px-4 py-3 text-sm font-medium transition-colors hover:bg-[var(--neutral-100)]"
            >
              Motion vs Anytype →
            </Link>
